...
class MainWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        # UI timer for instant updates (max speed)
        self.ui_timer = QTimer()
        self.ui_timer.timeout.connect(self._update)
        self.ui_timer.start(1)  # ~1000 Hz for instant response
        ...
        while self.ser and self.running:
            try:
                ...
                with self.data_lock:
                    ...
                    self.forces['FL'] = fl_val
                    self.forces['FR'] = fr_val
                    self.forces['RL'] = rl_val
                    self.forces['RR'] = rr_val
                    ...
                    if self.recording and self.csv_file:
                        try:
                            ...
                            pass
                # No sleep - process data as fast as possible for instant response
            except Exception:
                continue  # No delay on error either
    ...
    def _update(self):
        with self.data_lock:
            try:
                ...
                pass
            if self.trace:
                # Use numpy for faster array conversion
                xs = np.array([p[0] for p in self.trace])
                ys = np.array([p[1] for p in self.trace])
                try:
